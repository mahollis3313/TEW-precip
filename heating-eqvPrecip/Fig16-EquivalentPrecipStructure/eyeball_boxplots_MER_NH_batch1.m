...
%3-hourly 850 hPa wave tracks for N. HEMISPHERE
...
for y = 2001:2020
    ...
    lon=ncread(Ntrack_fname,'longitude'); lon=wrapTo180(lon);
    lat=ncread(Ntrack_fname,'latitude');
    time=ncread(Ntrack_fname,'time'); wave_count = wave_count + length(time);
...
var_name1 = strcat(folder,'NH_TEW_accumavgLH_700_',year,'.mat');
save(var_name1,'TEW_integ_heating')               

var_name2 = strcat(folder,'NH_TEW_latlon_counter_700_',year,'.mat');
